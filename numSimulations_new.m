...
for nr = 1:length(NR_vec)
    ...
    num_hat = mean(num_hat_t);
    num_hat_diag = mean(num_hat_diag_t);
    num_hat_diag_reg = mean(num_hat_diag_reg_t);

    den1_hat = mean(den1_hat_t);
    den2_hat = mean(den2_hat_t);
    den1_hat_diag = mean(den1_hat_diag_t);
    den2_hat_diag = mean(den2_hat_diag_t);
    den1_hat_diag_reg = mean(den1_hat_diag_reg_t);
    den2_hat_diag_reg = mean(den2_hat_diag_reg_t);

    den1_hat_DL = mean(den1_hat_DL_t);
    den1_hat_diag_DL = mean(den1_hat_diag_DL_t);
    den1_hat_diag_reg_DL = mean(den1_hat_diag_reg_DL_t);

    save(strcat(out_folder,'/outs_',string(nr), '_', string(nq_sim), '_', string(block_ID),'.mat'), num_hat, num_hat_diag,...
        num_hat_diag_reg, den1_hat, den2_hat, den1_hat_diag, den2_hat_diag, den1_hat_diag_reg, den2_hat_diag_reg,...
        den1_hat_DL, den1_hat_diag_DL, den1_hat_diag_reg_DL);
